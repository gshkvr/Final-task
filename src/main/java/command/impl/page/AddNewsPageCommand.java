...
public class AddNewsPageCommand implements Command {
    ...
    public Page execute(SessionRequestContent content) {
        content.setRequestAttribute(CURRENT_COMMAND, ADD_NEWS_PAGE_COMMAND);
        String error = content.getRequestParameter(ERROR_PARAMETER);
        if (error != null) {
            content.setRequestAttribute(CURRENT_COMMAND, ADD_NEWS_PAGE_COMMAND + ConfigurationManager.getProperty(error));
